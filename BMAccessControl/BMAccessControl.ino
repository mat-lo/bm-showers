//adapted 
//https://www.hackster.io/muhammad-aqib/rfid-based-access-control-system-using-arduino-de5359
...
void setup() {
  ...
  if (digitalRead(wipeB) == LOW) {  // when button pressed pin should get low, button connected to ground
    ...
    Serial.println("This will remove");    
    Serial.println("all records");    
    Serial.println("You have 10 ");   
    Serial.println("secs to Cancel");    
    Serial.println("Unpres to cancel");    
    ...
    if (buttonState == true && digitalRead(wipeB) == LOW) {    // If button still be pressed, wipe EEPROM
      ...
      Serial.println("Wiping Cancelled"); // Show some feedback that the wipe button did not pressed for 10 seconds
      digitalWrite(redLed, LOW);
    }
  }
  ...
  if (EEPROM.read(1) != 143) {
    ...
    Serial.println("Scan A Tag to define as master");    
    do {
  ...
  if (programMode) {
    if ( isMaster(readCard) ) { //When in program mode check First If master card scanned again to exit program mode      
      ...
      if ( findID(readCard) ) { // If scanned card is known delete it        
        ...
        deleteID(readCard);        
        Serial.println("Tag to ADD/REM");        
        Serial.println("Master to Exit");
      }
      else {                    // If scanned card is not known add it        
        Serial.println("New Tag,adding...");
        writeID(readCard);        
        Serial.println("Scan to ADD/REM");        
        Serial.println("Master to Exit");
      }
    }
  }
    ...
    if ( isMaster(readCard)) {    // If scanned card's ID matches Master Card's ID - enter program mode
      ...
      Serial.println("Scan a Tag to ADD/REMOVE");      
    }
    else {
...
void writeID( byte a[] ) {
  if ( !findID( a ) ) {     // Before we write to the EEPROM, check to see if we have seen this card before!
    ...
    Serial.println("wrong ID or bad EEPROM");
    delay(2000);
  }
}
